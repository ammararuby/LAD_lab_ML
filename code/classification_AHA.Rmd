...
importance_summary = importance_df %>%
  pivot_longer(cols = c("Musa":"Pistacia.vera"), names_to = "food", values_to = "importance") %>%
...
```{r}
input %>%
  select(group, Theobroma.cacao) %>%
  ggplot()+
  geom_boxplot(aes(x= group, y= Theobroma.cacao))+
  theme_classic()

input %>%
  select(group, Lauraceae) %>%
  ggplot()+
  geom_boxplot(aes(x= group, y= Lauraceae))+
  theme_classic()

input %>%
  select(group, Rosaceae.1) %>%
  ggplot()+
  geom_boxplot(aes(x= group, y= Rosaceae.1))+
  theme_classic()
