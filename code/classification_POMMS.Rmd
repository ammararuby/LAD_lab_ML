...
results = loop_label(iterations =20, input, label_list, percent = 0.7, upsampling = TRUE)
...
importance_summary = importance_df %>%
  pivot_longer(cols = c("NA.3":"NA.2"), names_to = "food", values_to = "importance") %>%
...
plot_food_direction(top10, features, variable = "group")
```
...
```{r}
cor_hel = feature_cor["Helianthus.annuus",] %>%
  as.data.frame()

colnames(cor_hel) = c("corr")
