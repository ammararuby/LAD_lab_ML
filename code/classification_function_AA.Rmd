...
loop_label = function(iterations, input, label_list, percent, upsampling){
  ...
  for(label in label_list){
    ...
    results = loop_model(iterations, current_data, label, percent, input_cf, auc_list, importance_df, upsampling)
...
# percent = percentage of train to test. specify as a double. Eg. 0.6.
# input_cf = running object of confusionmatrices, will be 0 at the very start. Normally specified through loop_label. User will have to specify as 0 if running this function separately.
# auc_list = running object of AUCs obtained over iterations, will be c() at the very start. Normally specified through loop_label. User will have to specify as c() if running this function separately.
# importance_df = running object of variable importance dataframes obtained over iterations, will be data.frame() at the very start. Normally specified through loop_label. User will have to specify as data.frame() if running this function separately.

####


loop_model = function(iterations, current_data, label, percent = 0.7, input_cf, auc_list, importance_df, upsampling){
  ...
  for (i in 1:iterations) {
    ...
    if(upsampling == TRUE){
      ...
      train = SMOTE(formula_smote, data  = train)
    }
