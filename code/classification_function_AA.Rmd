...
loop_label = function(iterations, input, label_list, percent= 0.7, upsampling = FALSE, downsampling = FALSE, repeated_measures = FALSE){
  ...
  for(label in label_list){
    ...
    if (repeated_measures == TRUE) {
      print("initiating repeated measures protocol")
      current_data = input %>%
    ...
    if (repeated_measures == TRUE) {
     print("running repeated measures model loop")
     results = loop_model_repeated_measures(iterations, current_data, label, percent, upsampling, downsampling)
...
## loop_model_repeated_measures
```{r}
loop_model_repeated_measures = function(iterations, current_data, label, percent= 0.7, upsampling = FALSE, downsampling = FALSE){
  ...
  for (i in 1:iterations) {
    ...
    #print(idlist) # remove before running multiple iterations
     
     trainsize = as.integer(percent * length(idlist))
     
     trainid = sample((idlist))[1:trainsize]
     
     #print(trainid) # remove before running multiple iterations
     
     train = current_data %>%
       filter(sample %in% trainid)
     
     group_folds = groupKFold(train$sample, k = 5)
    ...
    group_fit_control= trainControl(index = group_folds,
                                  method = "cv",
                                  summaryFunction = prSummary,
                             classProbs = T,
