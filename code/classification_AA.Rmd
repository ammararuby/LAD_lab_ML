...
results = loop_model(iterations = 2, input, label, percent, input_cf, auc_list, importance_df)
...
loop_model = function(iterations, input, label, percent = 0.7, input_cf, auc_list, importance_df){
  
  for (i in 1:iterations) {
    
    # Data splitting
    train_ind = createDataPartition((input %>%
    ...
    fit_control = trainControl(method = "CV", # 5-fold cross validation
                                 number = 5,
                                 summaryFunction = prSummary,
                                 classProbs = T)
    
    model = train(formula, #using formula specified 
              data = train,
                  method = "rf",
                  trControl = fit_control,
                  verbose = FALSE, 
    ...
    # Confusion matrix
    cf = confusionMatrix(data = pred_labels, reference =as.factor(predictions$obs), mode = "prec_recall")
    
    input_cf = input_cf + cf$table
    
    # Evaluate model
    
    eval = evalm(predictions, showplots = FALSE, silent = TRUE)
    
    auc_list = append(auc_list, eval$stdres$`Group1`[13,1])
    
    #Extract feature importance
    feature_importance = varImp(model$finalModel) %>% # Ben calls this importances instead of final model but the rankings are exactly the same, only number value scale is different.
      arrange(desc(Overall)) %>%
      as.matrix() %>%
      t()
